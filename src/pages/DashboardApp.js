      ...
      <Grid item className="main_grid">
        ...
        <Box height="10%" display="flex" flexDirection="column" alignItems="center" color="#fff" style={{ top: "7px", position: "relative" }}  >
          <Typography style={{ fontSize: "29px", width: "100%", justifyContent: "center", display: "flex" }}>
            ...
            <marquee style={{ fontFamily: "COLBO", fontSize: "1.8vw" }}>פטור ממדד לשכה / הטבה בשינויי דיירים! / ליווי אדריכלי לעיצוב / פטור משכ"טעו"ד / תנאי מימון 20/80 / פטור ממדד לשכה! / ליווי אדריכלי לעיצוב הדירה / הטבה בשיכויי</marquee>
          </Typography>
        </Box>
        <Box marginTop="27%" display="flex" flexDirection="column" alignItems="center" color="#fff" style={{ backgroundColor: '#232323', boxShadow: "0px -31px 126px 84px rgb(35 35 35)", height: "auto" }}>
          <Grid container style={{ width: "73vw" }}>
            <Grid item xs={7} style={{ display: "grid", justifyContent: "right", marginTop: "-2%", paddingRight: "6%", fontFamily: "COLBO" }}>
              ...
              <Typography style={{ textAlign: "right", fontFamily: "COLBO", marginTop: "1vw" }} className="bottom_text_3">רוצה לשמוע עוד?מלא פרטים ונחזור אלייך בהקדם</Typography>
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                <div className="input_div">
                  <label className="input_label_1"><span>:</span>מייל</label>
                  <input className="input_style" type="text"></input>
                </div>
                <div className="input_div">
                  <label className="input_label_1"><span>:</span>שם מלא</label>
                  <input className="input_style" type="text"></input>
                </div>
              </Stack>
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                <div className="input_div">
                  <label className="input_label_1"><span>{`<`}</span>פרויקט</label>
                  <input className="input_style" type="text"></input>
                </div>
                <div className="input_div">
                  <label className="input_label_1"><span>:</span>טלפון</label>
                  <input className="input_style" type="text"></input>
                </div>
              </Stack>
              <Stack direction={{ xs: 'column', sm: 'row' }} paddingTop={2} spacing={2}>
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} style={{ width: "50%", display: "display", direction: "rtl" }} spacing={2}>
                  <img alt="prop" style = {{height:"3vw"}} src="static/image/root.png"></img>
                </Stack>
              </Stack>
              <p style={{ justifyContent: "center", marginTop: "5%", paddingBottom: "1%", display: "inline-flex", direction: "rtl" }} className="bottom_text_3">ההטבות לחוזים שיחתמו עד ה- 31.12.21 בלבד!</p>
            </Grid>
